      ...
      <View style={styles.mainContent}>
        ...
        <ScrollView style={styles.listContainer} contentContainerStyle={{ paddingBottom: 50 }}>
          {TRANSACTIONS.map((txn, index) => (
            <Pressable key={index} style={({ hovered }: { hovered: boolean }) => [styles.transactionCard, hovered && styles.transactionCardHover]}>
...
const styles = StyleSheet.create({
  ...
  actionButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255, 255, 255, 0.2)', padding: 15, borderRadius: 12, marginBottom: 15, cursor: 'pointer', transition: 'transform 0.25s ease, background-color 0.25s ease, opacity 0.2s ease' } as any,
  actionButtonHover: { backgroundColor: 'rgba(255, 255, 255, 0.3)', transform: [{ translateY: -1 }] } as any,
  ...
  tabButton: { paddingVertical: 8, paddingHorizontal: 20, borderRadius: 15, cursor: 'pointer', transition: 'transform 0.2s ease, background-color 0.2s ease' } as any,
  tabButtonHover: { transform: [{ translateY: -1 }] } as any,
