...
export default function HomeScreen() {
  ...
  const [paymentStep, setPaymentStep] = useState<'selection' | 'details' | 'success'>('selection');
  ...
  const changePaymentStep = (newStep: 'selection' | 'details' | 'success') => {
  ...
  const filteredCards = DONATION_CARDS.filter((card) => {
    ...
    const searchableText = `${card.title} ${card.desc} ${card.amount}`.toLowerCase();
  ...
  return (
    <View style={[
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerRight}>
                ...
                <View style={styles.navDropdown}>
                  <Pressable 
                    style={({ hovered, pressed }: { hovered: boolean; pressed: boolean }) => [
                      ...
                      hovered && styles.navDropdownItemHover,
            ...
            <Pressable
              style={({ hovered, pressed }: { hovered: boolean; pressed: boolean }) => [
                styles.cartButton,
                hovered && styles.cartButtonHover,
        ...
        <View style={styles.mainContent}>
          ...
          <View style={styles.gridContainer}>
            {filteredCards.map((card) => (
              <Pressable
                key={card.id}
                style={({ hovered }: { hovered: boolean }) => [styles.card, hovered && styles.cardHover]}
              >
          ...
          {filteredCards.length === 0 && (
            <Text style={styles.noResultsText}>No donation cards matched your search.</Text>
          )}
        </View>
      </ScrollView>
      ...
      {/* ========================================= */}
      <View pointerEvents={isPaymentModalOpen ? 'auto' : 'none'} style={[styles.overlayContainer, { zIndex: 1001 }]}>
        <Pressable style={[styles.backdrop, { opacity: isPaymentModalOpen ? 1 : 0 }]} onPress={closePaymentModal} />
        
        <View style={[styles.paymentModal, { 
          opacity: isPaymentModalOpen ? 1 : 0, 
          transform: [{ scale: isPaymentModalOpen ? 1 : 0.95 }, { translateY: isPaymentModalOpen ? 0 : 20 }] 
        }]}>
            ...
            <View style={[styles.modalStepContainer, { opacity: isStepTransitioning ? 0 : 1, transform: [{ scale: isStepTransitioning ? 0.95 : 1 }] }]}>
              ...
              <Pressable 
                style={({ hovered, pressed }: { hovered: boolean; pressed: boolean }) => [
                  ...
                  hovered && paymentMethod && styles.continueButtonHover,
                  !paymentMethod && { opacity: 0.5 }, 
                  pressed && paymentMethod && { opacity: 0.85, transform: [{ scale: 0.98 }] }
                ]}
                onPress={() => {
                  if (paymentMethod) changePaymentStep('details');
                }}
              >
          ...
          {paymentStep === 'details' && (
            <View style={[styles.modalStepContainer, { opacity: isStepTransitioning ? 0 : 1, transform: [{ scale: isStepTransitioning ? 0.95 : 1 }] }]}>
              <Pressable style={styles.backButton} onPress={() => changePaymentStep('selection')}>
                <Text style={styles.backButtonText}>← Back</Text>
              </Pressable>

              <Text style={styles.paymentModalTitle}>Enter {paymentMethod} Details</Text>

              {(paymentMethod === 'Gcash' || paymentMethod === 'Paymaya') ? (
                <View style={styles.formWrapper}>
                  <Text style={styles.inputLabel}>Mobile Number</Text>
                  <TextInput style={styles.modalInput} placeholder="09XX XXX XXXX" keyboardType="phone-pad" placeholderTextColor="#A0A0A0" />
                </View>
              ) : (
                <View style={styles.formWrapper}>
                  <Text style={styles.inputLabel}>Card Number</Text>
                  <TextInput style={styles.modalInput} placeholder="XXXX XXXX XXXX XXXX" keyboardType="number-pad" placeholderTextColor="#A0A0A0" />
                  <View style={styles.rowInputs}>
                    <View style={{ flex: 1, marginRight: 15 }}>
                      <Text style={styles.inputLabel}>Expiry Date</Text>
                      <TextInput style={styles.modalInput} placeholder="MM/YY" placeholderTextColor="#A0A0A0" />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.inputLabel}>CVV</Text>
                      <TextInput style={styles.modalInput} placeholder="123" keyboardType="number-pad" secureTextEntry placeholderTextColor="#A0A0A0" />
                    </View>
                  </View>
                  <Text style={styles.inputLabel}>Cardholder Name</Text>
                  <TextInput style={styles.modalInput} placeholder="Juan Dela Cruz" placeholderTextColor="#A0A0A0" />
                </View>
              )}
              ...
              <Pressable 
                style={({ hovered, pressed }: { hovered: boolean; pressed: boolean }) => [
                  styles.continueButton, 
                  hovered && styles.continueButtonHover,
                  pressed && { opacity: 0.85, transform: [{ scale: 0.98 }] }
                ]}
                onPress={() => {
              ...
              >
                <Text style={styles.continueButtonText}>Pay {calculateTotal()}</Text>
              </Pressable>
            </View>
          )}
          ...
          {paymentStep === 'success' && (
            <View style={[styles.modalStepContainer, { opacity: isStepTransitioning ? 0 : 1, transform: [{ scale: isStepTransitioning ? 0.95 : 1 }] }]}>
              ...
              <View style={styles.shareBoxContainer}>
                <TextInput style={styles.shareInput} value="https://example.com/test-endpoint" editable={false} />
                <Pressable onPress={() => console.log('Copied!')}>
                  <CopyIcon color="#888" size={18} />
                </Pressable>
              </View>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  menuButton: { flexDirection: 'row', alignItems: 'center', marginRight: 25, cursor: 'pointer', transition: 'transform 0.22s ease, opacity 0.2s ease' } as any,
  menuButtonHover: { transform: [{ translateY: -1 }] } as any,
  ...
  navDropdownItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 15, paddingHorizontal: 25, cursor: 'pointer', transition: 'background-color 0.2s ease' } as any,
  navDropdownItemHover: { backgroundColor: '#FAFAFA' } as any,
  ...
  cartButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#5A1010', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 25, cursor: 'pointer', zIndex: 1100, transition: 'transform 0.25s ease, opacity 0.2s ease, box-shadow 0.25s ease' } as any,
  cartButtonHover: { transform: [{ translateY: -1 }], boxShadow: '0px 8px 18px rgba(90, 16, 16, 0.28)' } as any,
  ...
  searchInput: { flex: 1, fontSize: 16, color: '#5F5E5E', outlineStyle: 'none' } as any,
  noResultsText: { fontSize: 15, color: '#888', textAlign: 'center', marginTop: 5 },

  gridContainer: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', gap: 30 } as any,
  card: { width: '31%', minWidth: 300, backgroundColor: '#FFFFFF', borderRadius: 20, overflow: 'hidden', boxShadow: '0px 5px 15px rgba(0, 0, 0, 0.08)', marginBottom: 10, borderWidth: 1, borderColor: '#F0F0F0', transition: 'transform 0.28s ease, box-shadow 0.28s ease' } as any,
  cardHover: { transform: [{ translateY: -3 }], boxShadow: '0px 12px 24px rgba(0, 0, 0, 0.12)' } as any,
  ...
  addButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#6A1B1B', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 20, cursor: 'pointer', transition: 'transform 0.22s ease, opacity 0.2s ease' } as any,
  addButtonHover: { transform: [{ translateY: -1 }] } as any,
  ...
  cartItemsList: { flexShrink: 1, padding: 30 },
  ...
  checkoutSection: { flexGrow: 1, flexShrink: 0, backgroundColor: '#F3F4F6', padding: 30, paddingBottom: 40, borderTopWidth: 1, borderTopColor: '#E5E7EB' },
  ...
  checkoutButton: { height: 55, width: '100%', borderRadius: 30, justifyContent: 'center', alignItems: 'center', backgroundImage: 'linear-gradient(to right, #D9898C, #B75658)', boxShadow: '0px 8px 20px rgba(183, 86, 88, 0.3)', cursor: 'pointer', transition: 'transform 0.25s ease, box-shadow 0.25s ease, opacity 0.2s ease' } as any,
  checkoutButtonHover: { transform: [{ translateY: -1 }], boxShadow: '0px 12px 24px rgba(183, 86, 88, 0.35)' } as any,
  ...
  paymentModal: { width: 450, backgroundColor: '#FFFFFF', borderRadius: 24, padding: 40, boxShadow: '0px 15px 40px rgba(0, 0, 0, 0.1)', zIndex: 1201, transition: 'all 0.4s cubic-bezier(0.25, 0.8, 0.25, 1)' } as any,
  modalStepContainer: { transition: 'all 0.25s ease-in-out' } as any,
  backButton: { position: 'absolute', top: -10, left: 0, padding: 10, zIndex: 10, cursor: 'pointer' } as any,
  ...
  formWrapper: { marginBottom: 30 },
  inputLabel: { fontSize: 13, fontWeight: '600', color: '#6A1B1B', marginBottom: 8, marginLeft: 4 },
  modalInput: { borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 12, padding: 16, fontSize: 15, backgroundColor: '#F9F9F9', marginBottom: 20, outlineStyle: 'none' } as any,
  rowInputs: { flexDirection: 'row', justifyContent: 'space-between' },
  continueButton: { height: 55, width: '100%', borderRadius: 30, justifyContent: 'center', alignItems: 'center', backgroundImage: 'linear-gradient(to right, #D9898C, #B75658)', boxShadow: '0px 8px 20px rgba(183, 86, 88, 0.3)', cursor: 'pointer', transition: 'transform 0.25s ease, box-shadow 0.25s ease, opacity 0.2s ease' } as any,
  continueButtonHover: { transform: [{ translateY: -1 }], boxShadow: '0px 12px 24px rgba(183, 86, 88, 0.35)' } as any,
  ...
  successMessage: { fontSize: 18, fontWeight: 'bold', color: '#333', textAlign: 'center', marginBottom: 35 },
